...
SET @NEXT_SEQ = (SELECT (SHLANG.SEQ_NUM+1) FROM PSGACTE.DBO.SHLANG 
				WHERE SHLANG.TP_OBJ = 'IMANIDF' AND SHLANG.OBJ = 'IMANIDF.COD_MANIFESTO')
SET @COD_MDFE	= REPLICATE('0', 8 - LEN(@NEXT_SEQ)) + RTrim(@NEXT_SEQ)
SET @NR_MDFE	= (SELECT (CSERIES.ULT_NUM+1) FROM PSGACTE.DBO.CSERIES WHERE CSERIES.SERIE = 'MDFE')


UPDATE PSGACTE.DBO.SHLANG
	SET SHLANG.SEQ_NUM = @NEXT_SEQ
...
VALUES(@COD_MDFE,@NR_MDFE,'MDFE','2',@CHAVENOTA)

INSERT INTO PSGACTE.DBO.IMANCONT (COD_MANIFESTO,SERIE_MANIFESTO,COD_PESSOA)
VALUES(@COD_MDFE,'MDFE','04726')
